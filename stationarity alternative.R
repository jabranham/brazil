...
#alternative adf function based on tseries adf.test but with regression saved to output
my.adf.copy <- function (x, alternative = c("stationary", "explosive"), k = trunc((length(x) - 
                                                                      1)^(1/3))) 
{
  if (NCOL(x) > 1) 
  ...
  structure(list(statistic = STAT, parameter = PARAMETER, alternative = alternative, 
                 p.value = PVAL, method = METHOD, data.name = DNAME, regression = res.sum), 
            class = "htest")
}

for (i in 9:0) {
  foo <- my.adf.copy(w.list$Proportion.Rousseff, k=i)
  print(foo[1:7])
}

for (i in 9:0) {
  foo <- my.adf.copy(w.list$Proportion.Neves, k=i)
  print(foo[1:7])
}

for (i in 9:0) {
  foo <- my.adf.copy(w.list$Proportion.PSB, k=i)
  print(foo[1:7])
}

for (i in 9:0) {
  foo <- my.adf.copy(w.list$Proportion.DK, k=i)
  print(foo[1:7])
}

for (i in 9:0) {
  foo <- my.adf.copy(w.list$Proportion.Other, k=i)
  print(foo[1:7])
}
